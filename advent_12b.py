...
class Visited(object):
  def __init__(self, cave, visited, small_twice):
    self.cave = cave
...
q.put(Visited(caves["start"], ["start"], False))
...
while not q.empty():
  ...
  for n in current.cave.neighbors:
    if n == "end":
      ...
      small_twice = current.small_twice
      cont = n.isupper() or n not in current.visited
      if not cont and n != "start" and not small_twice:
        small_twice = True
        cont = True
      if cont:
        new_visited = copy.deepcopy(current.visited) + [n]
        q.put(Visited(caves[n], new_visited, small_twice))
