  ...
  for j in range(10):
    for i in range(10):
      if board[j][i] > 9 and (get_index(i, j) not in flashed):
        # If flash not yet counted, count flesh, update flashed-neighbor counter
        # for all neighbors of the flashing node.
        flashed.add(get_index(i, j))
        for n in cell_to_neighbors[get_index(i, j)]:
          flashed_n_counts[n] += 1
  for j in range(10):
    for i in range(10):
      # Score increased by the count of the neighbors that just flashed
